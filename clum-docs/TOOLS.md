...
| `keys` | string | ✅ |
| `sensitive` | boolean | | 标记输入为敏感内容，审计 detail 脱敏为 `[REDACTED:N bytes]` |

> 终端处于 `password` 状态时发送的输入**无条件自动脱敏**（无法关闭），审计事件带 `redacted: true` 标志，detail 附服务端提取的提示行上下文 `(prompt: "...")`。
...
| `action` | string | 操作类型（69 种 AuditAction） |
| `detail` | string | 操作参数 |
